...
import {default as imageMin} from 'gulp-imagemin';
...
function cacheBustTask(done){
    ...
    return src(files.htmlDist)
        .pipe(replace(/[?]cachebuster=\d+/g, '?cachebuster=' + cbString)) // Replace current time stamp CSS query string in the HTML files with the new time stamp
...
function cleanTask(done){
    deleteAsync([`${folders.dist}/**`, `!${folders.dist}`]);
    done();
}

// Default Task:
//exports.default = series(parallel(sassTask, jsTask), watchTask);
//task('default', series(parallel(sassTask, jsTask), watchTask));
...
//task('build', series(setBuildState, cleanTask, parallel(sassBuildTask, jsBuildTask, jQueryTask), parallel(imagesTask, mediaTask, fontsTask), htmlTask, cacheBustTask))
const buildTask = series(cleanTask, parallel(sassBuildTask, jsBuildTask, jQueryTask), parallel(imagesTask, mediaTask, fontsTask), htmlTask, cacheBustTask);
